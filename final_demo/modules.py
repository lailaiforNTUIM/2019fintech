...
def get_coshow(contents):
	coshow_dict = {}
	pre_pos = ["a","ad","Ag","d","g","h","m","q","v"]
	post_pos = ["an","j","k","n","nr","ns","nt","nz","tg","t","un"]
	cat_content = ' '.join(contents)
	clean_content = remove_punctuation(cat_content)
	cut_content = pseg.lcut(clean_content)
	cut_content = list(filter(lambda x: x[0]!=' ', cut_content))
	for i in range(len(cut_content)-1):
		pos0,pos1 = cut_content[i][1],cut_content[i+1][1]
		if(pos0 in pre_pos and pos1 in post_pos):
			wcw = cut_content[i][0] + cut_content[i+1][0]
			coshow_dict[wcw] = coshow_dict.get(wcw,0)+1
